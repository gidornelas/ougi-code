...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const dirs = unique(directories).filter(
    (dir) => ConfigPaths.LOCAL_CONFIG_DIRS.some((item) => dir.endsWith(item)) || dir === Flag.OPENCODE_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!ConfigPaths.LOCAL_CONFIG_DIRS.some((item) => dir.endsWith(item)) && dir !== Flag.OPENCODE_CONFIG_DIR) continue
