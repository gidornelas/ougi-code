...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run ougi",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for share link in PR body
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await Process.text(["ougi", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting ougi...")
        UI.println()

        const ougiArgs = sessionId ? ["-s", sessionId] : []
        const ougiProcess = Process.spawn(["ougi", ...ougiArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await ougiProcess.exited
        if (code !== 0) throw new Error(`ougi exited with code ${code}`)
      },
    })
  },
})
