...
for (const [key, value] of Object.entries(process.env)) {
  if (!key.startsWith("OUGI_") || value === undefined) continue
  const legacy = `OPENCODE_${key.slice("OUGI_".length)}`
  process.env[legacy] ??= value
}

function env(key: string) {
  if (!key.startsWith("OPENCODE_")) return process.env[key]
  const alias = `OUGI_${key.slice("OPENCODE_".length)}`
  return process.env[alias] ?? process.env[key]
}
