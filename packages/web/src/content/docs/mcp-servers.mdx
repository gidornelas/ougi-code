...
Ougi automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Ougi will:
...
If the server requires authentication, Ougi will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `ougi mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Ougi will store the tokens securely in `~/.local/share/ougi/mcp-auth.json`.
...
ougi mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Ougi, alongside built-in tools. So you can manage them through the Ougi config like any other tool.
