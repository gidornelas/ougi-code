...
There are two types of agents in Ougi; primary agents and subagents.
...
Ougi comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Ougi comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Ougi comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `ougi.json` config file:

```json title="ougi.json"
{
...
- Global: `~/.config/ougi/agents/`
- Per-project: `.ougi/agents/`

```markdown title="~/.config/ougi/agents/review.md"
---
...
If no temperature is specified, Ougi uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Ougi config and the project specific config.
...
The model ID in your Ougi config uses the format `provider/model-id`. For example, if you're using the hosted compatibility provider, you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `ougi models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/gidornelas/ougi-code).
:::
...
```markdown title="~/.config/ougi/agents/docs-writer.md"
---
...
```markdown title="~/.config/ougi/agents/security-auditor.md"
---
