---
title: Providers
description: Using any LLM provider in Ougi Code.
---

import config from "../../../config.mjs"
export const console = config.console

Ougi uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers**, including local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Ougi config.
...
in `~/.local/share/ougi/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Ougi
config.
...
Ougi Connect is the fork's recommended hosted entry point. It uses the upstream-compatible provider service and gives you a quick way to get started with tested coding models. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Ougi Connect.
:::
...
It works like any other provider in Ougi and is completely optional to use.

---

## OpenCode Go

OpenCode Go is the upstream-compatible low-cost subscription plan surfaced by this fork for reliable access to popular open coding models.

1. Run the `/connect` command in the TUI, select `OpenCode Go`, and head to [opencode.ai/zen](https://opencode.ai/zen).
