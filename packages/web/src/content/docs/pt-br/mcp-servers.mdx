...
Se o servidor requer autenticação, o ougi solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `ougi mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o ougi armazenará os tokens de forma segura em `~/.local/share/ougi/mcp-auth.json`.
